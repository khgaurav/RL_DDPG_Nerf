...
class DDPG(object):
    ...
    def learn(self, total_timesteps=10000, callback=None):
        ...
        state = np.concatenate([obs_dict[key].flatten() for key in obs_dict.keys()])
        terminated = False
        truncated = False
        total_reward = 0
        episode = 0
        ...
        for t in pbar:
            ...
            next_obs_dict, reward, terminated, truncated, info = env.step(action.flatten())
            # Concatenate next observation dictionary
            next_state = np.concatenate([next_obs_dict[key].flatten() for key in next_obs_dict.keys()])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", default="PandaReachJointsDense-v3")
    ...
    # Optimal Parameters without priority: {'actor_lr': 0.000707753683741106, 'critic_lr': 0.0053198850838189265, 'gamma': 0.9576430682266931, 'tau': 0.007565445872139265, 'epsilon': 0.47731749204678015, 'batch_size': 174, 'buffer_size': 60166.808882182784}
    # otimal with priority 
    params = {'actor_lr': 0.0007046881317658853, 'critic_lr': 0.0024931027321080284, 'gamma': 0.9662992583584146, 'tau': 0.008678664044526942, 'epsilon': 0.39138840938325403, 'batch_size': 125, 'buffer_size': 52300.60687495506, 'hidden_dims': [256, 400]}
    #PandaReachJointsDense-v3 
    # params = {'actor_lr': 0.00038848055094551167, 'critic_lr': 0.0018576290719221995, 'gamma': 0.9, 'tau': 0.009078396426970179, 'epsilon': 0.39671772979241005, 'batch_size': 125, 'buffer_size': 62156.373157132286}
    ...
    if args.test:
        ...
        if args.param_optimize:
            optimizer = WhaleOptimizer(num_whales=30, max_iter=30, timesteps=5000)

...
# Replace the existing environment creation code in training_agent.py
from MujocoEnv import FR3RLEnv, FR3Encoder
...
class DDPG(object):
    ...
    def learn(self, total_timesteps=10000, callback=None):
        ...
        previous_timestep = 0
        obs = env.reset()
        state = obs  # Direct use of encoded state
        terminated = False
        truncated = False
        total_reward = 0
        episode = 0
        ...
        for t in pbar:
            ...
            next_state, reward, terminated, info = env.step(action.flatten())
            ...
            self.buffer.extend(TensorDict({
                ...
                "priority": torch.ones(1, device=device)
            }, batch_size=[1]))
...
def plot_all_curves(curves, titles, xlabel, ylabel, fig_title):
    """
    Args:
...
class WhaleOptimizer:
    def __init__(self, num_whales=30, max_iter=30):
    ...
    def evaluate_fitness(self, params):
        # Create DDPG agent with current parameters
        print(f"Evaluating Parameters: {params}")
        agent = DDPG(state_dim, action_dim, params)
        agent.learn(total_timesteps=5000)  # Shorter evaluation for optimization
        return np.mean(agent.episode_rewards[-10:])  # Average of last 10 episodes
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", default="PandaReachJoints-v3")
    ...
    # Encoder
    params = {'actor_lr': 0.0009756377138895141, 'critic_lr': 0.002350765808327592, 'gamma': 0.9050823697282884, 'tau': 0.0027003648724279834, 'epsilon': 0.388890977703842, 'batch_size': 77, 'buffer_size': 20320.06659586964}

    if args.test:
        # Create Isaac Lab environment
        env = gym.make(args.env, render_mode='human')
        state_dim = sum([env.observation_space[key].shape[0] for key in env.observation_space.keys()])
        ...
        env = FR3RLEnv("fr3.xml", encoder)  # Make sure fr3.xml is in the correct path
        # Set seed for reproducibility
        seed = np.random.randint(0, 1000)
        print(f"Seed: {seed}")
        np.random.seed(seed)
        torch.manual_seed(seed)
        state_dim = env.observation_space.shape[0]  # Should be 64 (latent_dim)
        action_dim = env.action_space.shape[0]      # Should be 7 (FR3 joints)
        print(f"State Dimension: {state_dim}, Action Dimension: {action_dim}")
        # Initialize WOA
        ...
        for i in range(int(args.runs)):
            ...
            if(args.save):
                agent.save(args.env + f"_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.pth")
        ...
        plot_all_curves(curves, titles, ["Episodes", "Timestep", "Timestep", "Timestep", "Episodes"], ["Rewards", "Loss", "Loss", "Q-Value", "Timesteps"], str(args) + str(params))
